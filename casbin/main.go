...
func main() {
	...
	added, err = e.AddRoleForUser("role00", "role1")
	added, err = e.AddRoleForUser("role1", "role2")


	added, err = e.AddPolicy("role2", "data1", "read")
	e.AddFunction("self_match", KeyMatchFunc)
	fmt.Println(added)
	fmt.Println(err)
	results, err := e.BatchEnforce([][]interface{}{{"role1", "data1", "read"}})
